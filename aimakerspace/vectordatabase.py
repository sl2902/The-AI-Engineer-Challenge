...
class VectorDatabase:
    ...
    def search(
        ...
        if metadata_filter:
            ...
            print(f"Metadata filter applied: {total_items} total items -> {len(filtered_items)} filtered items")
        else:
            filtered_items = list(self.vectors.items())
            print(f"No metadata filter: using all {len(filtered_items)} items")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        """Get performance statistics for the vector database."""
        return {
            "total_vectors": len(self.vectors),
            "ann_enabled": False,
            "index_type": "basic",
            "index_trained": True,
            "memory_usage_estimate": len(self.vectors) * 1536 * 4  # 4 bytes per float32, 1536 dimensions
        }
